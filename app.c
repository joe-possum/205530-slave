  ...
  while (1) {
    ...
    if(memcmp(backup,bg_pool_pools,sizeof(backup))) {
    	if(backup[bg_pool_object_bgbuf].free != bg_pool_pools[bg_pool_object_bgbuf].free){
    		//if(!evt) printf("\e[21F"); // overwrite is no new events
    ...
    case gecko_evt_le_connection_opened_id:
  	  ...
  	  advertising = 0;
  	  gecko_cmd_hardware_set_soft_timer(2<<15,0,1);
  	  break;
...
#define ED evt->data.evt_hardware_soft_timer
			...
			case 1:
				...
				break;
			case 2: // heart beat
				gecko_cmd_gatt_server_write_attribute_value(gattdb_device_name,0,gattdb_device_name,(uint8*)"Hello, World!");
...
#undef ED
			...
			case gecko_evt_le_connection_closed_id:
				gecko_cmd_hardware_set_soft_timer(0,1,1);
				start_advertising();
				break;
